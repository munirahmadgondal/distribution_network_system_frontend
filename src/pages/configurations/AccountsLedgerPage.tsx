...
export function AccountsLedgerPage({kind,onBack}:{kind:'expenses'|'income';onBack:()=>void}){
  ...
  return <div>
    <div className="crud-header retailer-dispatch-page-header"><div><div className="retailer-dispatch-title-row"><Button type="text" icon={<ArrowLeftOutlined/>} onClick={onBack}/><Typography.Title level={3}>{data?.title||'Ledger'}</Typography.Title></div><Typography.Text className="retailer-dispatch-subtitle">View transactions and running balance.</Typography.Text></div></div>
